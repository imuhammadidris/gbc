<?php
if(!class_exists("CryptoxBase")) {
	class CryptoxBase {
    	public $key = "86813CD455E4C9FF";
    	private $product_id = "11";
    	private $product_base = "Cryptox WordPress";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_Cryptox WordPress_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=Cryptox WordPress_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );

...
if ( ! class_exists('Cryptex_WC_Nb') ) {
	...
	class Cryptex_WC_Nb {
		...
		public function __construct() {
			...
			$this->settings = array(
				array(
					'name' => esc_html__( 'New Badge', 'cryptox' ),
					'type' => 'title',
					'id' => 'wc_nb_options'
				),
				array(
					'name'   => esc_html__( 'Enable label new', 'cryptox' ),
					'desc'    => esc_html__( 'Visible label new in products', 'cryptox' ),
					'id'      => 'wc_nb_label_new',
					'default' => 'no',
					'type'    => 'checkbox'
				),
				array(
					'name' 		=> esc_html__( 'Product Newness', 'cryptox' ),
					'desc' 		=> esc_html__( "Display the 'New' flash for how many days?", 'cryptox' ),
					'id' 		=> 'wc_nb_newness',
					'type' 		=> 'number',
				),
		...
		public static function woocommerce_show_product_loop_new_badge() {
			...
			if ( ( time() - ( 60 * 60 * 24 * $newness ) ) < $postdatestamp ) { // If the product was published within the newness time frame display the new badge
				echo '<span class="new-badge">' . esc_html__( 'New', 'cryptox' ) . '</span>';
			}
		}
	}
